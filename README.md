...
第 9 階段目標是整理目前功能，讓 Demo 時可以清楚展示「資料 CRUD → 模型訓練 → 模型狀態 → 調整超參數 → 模型保護機制 → 單筆預測」的完整流程。
...
| 2 | 使用首頁功能 | 展示搜尋、分頁、新增、編輯與刪除資料 |
| 3 | 點擊「機器學習頁面」或進入 `/ml` | 進入模型訓練頁面 |
| 4 | 觀察「目前模型狀態」 | 顯示目前是否已有正式模型 |
| 5 | 調整超參數 | 修改 `n_estimators`、`max_depth`、`min_samples_split`、`test_size`、`random_state` |
| 6 | 按下「開始訓練模型」 | 前端使用 Fetch 呼叫 `POST /api/ml/train` |
| 7 | 查看訓練結果 | 顯示本次 Accuracy、舊模型 Accuracy、訓練參數與特徵欄位 |
| 8 | 查看模型是否取代 | 若本次 Accuracy 較高才更新正式模型，否則保留舊模型 |
| 9 | 進入 `/ml/predict` | 進入單筆乘客生還預測頁面 |
...
```text
這個專案一開始是 Titanic 乘客資料 CRUD 系統，後端使用 Flask，資料庫使用 SQLite，前端使用 HTML 與 JavaScript Fetch 呼叫 RESTful API。

後來我在原本 CRUD 架構上加入機器學習功能。使用者可以進入 /ml 頁面調整 Random Forest 的超參數，按下一鍵訓練後，後端會從 SQLite 的 titanic 資料表讀取資料，完成缺失值處理、one-hot encoding、資料切分、模型訓練與 Accuracy 評估。
...
| 測試項目 | 操作方式 | 預期結果 |
|---|---|---|
| 查看乘客列表 | 開啟首頁 `/` | 顯示 Titanic 乘客資料表 |
| 分頁功能 | 點擊上一頁 / 下一頁 | 可以切換不同頁面的乘客資料 |
| 搜尋功能 | 輸入乘客姓名關鍵字後查詢 | 顯示符合關鍵字的乘客資料 |
| 新增乘客 | 點擊「新增乘客」並送出表單 | 新增成功後回到首頁，資料庫新增一筆資料 |
| 編輯乘客 | 點擊指定乘客的「編輯」 | 可以修改乘客資料，更新後回到首頁 |
| 刪除乘客 | 點擊指定乘客的「刪除」 | 刪除成功後重新載入目前頁面 |
...
| 測試項目 | 操作方式 | 預期結果 |
|---|---|---|
| 尚未訓練模型 | 刪除或改名 `models/titanic_model.joblib` 或 `models/model_info.json` 後開啟 `/ml` | 顯示尚未訓練模型 |
| 已訓練模型 | 訓練完成後重新開啟 `/ml` | 顯示模型已訓練完成、Accuracy 與參數資訊 |
| 狀態 API 測試 | 開啟 `/api/ml/status` | 回傳 `trained: true` 或 `trained: false` |
| 模型資訊檢查 | 模型存在時查看 `/api/ml/status` | 回傳正式模型的 Accuracy、參數、特徵欄位與訓練時間 |
...
| 測試項目 | 操作方式 | 預期結果 |
|---|---|---|
| 點擊訓練按鈕 | 在 `/ml` 點擊「開始訓練模型」 | 後端開始訓練 Random Forest 模型 |
| 顯示訓練中 | 按下按鈕後觀察頁面 | 頁面顯示模型訓練中，按鈕暫時不可重複點擊 |
| 顯示訓練結果 | 訓練完成後觀察頁面 | 顯示 Accuracy、資料筆數、特徵欄位與訓練參數 |
| 儲存模型 | 本次 Accuracy 較高時檢查 `models/` 資料夾 | 更新 `titanic_model.joblib` 與 `model_info.json` |
...
| 測試項目 | 操作方式 | 預期結果 |
|---|---|---|
| 調整 `n_estimators` | 在 `/ml` 修改樹的數量 | 後端使用新的樹數量訓練 |
| 調整 `max_depth` | 修改樹的最大深度 | 後端使用新的最大深度訓練 |
| 調整 `min_samples_split` | 修改節點分裂最小樣本數 | 後端使用新的分裂條件訓練 |
| 調整 `test_size` | 修改測試集比例 | 訓練集與測試集筆數會跟著改變 |
| 調整 `random_state` | 修改隨機種子 | 資料切分與模型結果可能改變 |
| 顯示本次參數 | 訓練完成後查看訓練結果表格 | 頁面顯示本次使用的所有超參數 |
...
| 測試項目 | 操作方式 | 預期結果 |
|---|---|---|
| 第一次訓練 | 尚無正式模型時按下訓練 | 本次模型會成為第一個正式模型 |
| 新模型較好 | 調整參數使本次 Accuracy 高於舊模型 | 系統更新正式模型與 `model_info.json` |
| 新模型沒有較好 | 調整參數使本次 Accuracy 小於或等於舊模型 | 系統保留原本正式模型 |
| 前端顯示比較結果 | 查看 `/ml` 訓練結果區塊 | 顯示舊 Accuracy、本次 Accuracy、是否比舊模型更好、是否取代目前模型 |
| 狀態 API 確認 | 訓練後開啟 `/api/ml/status` | 顯示目前正式模型資訊，而不是較差的新模型資訊 |
...
| 測試項目 | 操作方式 | 預期結果 |
|---|---|---|
| 尚未訓練就預測 | 沒有模型檔案時進入 `/ml/predict` 並送出表單 | 顯示請先訓練模型 |
| 輸入乘客資料 | 在 `/ml/predict` 填寫表單 | 可以送出資料給後端 |
| 預測是否生還 | 點擊「開始預測」 | 顯示生還或未生還 |
| 顯示生還機率 | 預測完成後查看結果 | 顯示生還機率百分比 |
| 特徵欄位一致 | 使用不同 `Sex` 與 `Embarked` 測試 | 預測時不會再出現 feature names 不一致錯誤 |
...
|---|---:|---|---:|---:|---:|---:|---|---|
| 測試 A | 1 | female | 30 | 0 | 0 | 100 | C | 通常會有較高生還機率 |
| 測試 B | 3 | male | 30 | 0 | 0 | 8 | S | 通常會有較低生還機率 |

注意：實際結果會受到模型參數、資料切分與目前正式模型影響，因此 Demo 時以頁面實際顯示的預測結果為準。
...
12. 完成第 9 階段：整理 Demo 流程、測試案例與 README。
